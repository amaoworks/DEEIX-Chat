...
`@deeix/web` 是 DEEIX Chat 的浏览器端工作区，基于 Next.js App Router 实现对话、文件、知识库、提示词、用户设置、站内私聊浮窗和管理员后台。前端负责界面、客户端状态和展示层流程；认证授权、模型路由、文件处理、计费、持久化和审计等业务规则由 Go 后端负责。

前端采用 Next.js 静态导出模式（`output: "export"`）。开发时使用 Next.js 开发服务器，构建后生成 `frontend/out`，生产环境可由 Go 服务托管，也可以交给 Nginx、CDN 或其他静态文件服务。

相关文档：[项目主 README](../README.md) · [后端 README](../backend/README.md) · [API 文档索引](../backend/docs/README.md) · [站内消息](../docs/INTERNAL_MESSAGING.md)
...
│   ├── chat/                  # 对话工作区
│   ├── files/                 # 文件管理与处理状态
│   ├── internal-messaging/    # 站内私聊浮窗、会话列表、实时事件和附件
│   ├── knowledge-bases/       # 知识库
│   ├── layouts/               # 工作区布局
│   ├── prompts/               # 提示词
│   ├── recent/                # 最近会话
