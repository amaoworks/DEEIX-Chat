...
// initialConfig 可指定首次启动时的站内消息开关；数据库已有设置始终优先。
func (s *Service) Seed(ctx context.Context, initialConfig ...config.Config) error {
	defaults := defaultSettings()
	if len(initialConfig) > 0 {
		for i := range defaults {
			if defaults[i].Namespace == "internal_messaging" && defaults[i].Key == "enabled" {
				defaults[i].Value = strconv.FormatBool(initialConfig[0].InternalMessagingEnabled)
			}
		}
	}
	items, err := s.encryptSettingsForStorage(defaults)
	if err != nil {
		return err
	}
